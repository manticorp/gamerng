...
return"a"===d?f.call(a,c):f?f.value=c:b.set(a,c),c};const U=/^ *([+-]? *[0-9_]*) *[dD] *([0-9_]+) *([+-]? *[0-9_.]*) *$/,M={},x={};let H=0,R=0,V=Date.now(),N=0;class W extends Error{}class O{constructor(a){this.version="0.2.3";F.set(this,0);D.set(this,void 0);G.set(this,void 0);da.set(this,"normal gaussian boxMuller irwinHall bates batesgaussian bernoulli exponential pareto poisson hypergeometric rademacher binomial betaBinomial beta gamma studentsT wignerSemicircle kumaraswamy hermite chiSquared rayleigh logNormal cauchy laplace logistic".split(" "));
...
F,"f"),version:"0.2.3"}}static unserialize(a,b=!1){var c;if((new w(null!==(c=a.version)&&void 0!==c?c:"0.1.0")).lt("0.2.0")&&!b)throw Error(`Trying to unserialize old RNG (v${a.version}) can lead to unexpected behaviour - minimum supported version for this iteration is ${"0.2.0"}.
...
D,"f")?z(this,D,"f").call(this):this._next()}percentage(){return this.randBetween(0,100)}probability(){return this.randBetween(0,1)}random(a=0,b=1,c=0){return this.randBetween(a,b,c)}chance(a,b=1){e({chanceIn:b}).positive();e({n:a}).positive();a/=b;return this._random()<=a}chanceTo(a,b){return this.chance(a,a+b)}randInt(a=0,b=1,c=0){"object"===typeof a&&({from:a=0,to:b=1,skew:c=0}=a);e({from:a}).int();e({to:b}).int();if(a===b)return a;[a,b]=[Math.min(a,b),Math.max(a,b)];let d=this._random();d=0>c?
1-Math.pow(d,Math.pow(2,c)):Math.pow(d,Math.pow(2,-c));return Math.floor(d*(b+1-a))+a}uniqid(a=""){return k(a)}static uniqid(a=""){return k(a)}randomString(a=6){e({len:a}).gt(0);const b=[];for(let c=0;c<a;c++)b.push("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"[this.randInt(0,61)]);return b.join("")}randBetween(a=0,b,c=0){"object"===typeof a&&({from:a=0,to:b,skew:c=0}=a);"undefined"===typeof b&&(b=a+1);[a,b]=[Math.min(a,b),Math.max(a,b)];let d=this._random();0>c?d=1-Math.pow(d,
Math.pow(2,c)):0<c&&(d=Math.pow(d,Math.pow(2,-c)));return this.scaleNorm(d,a,b)}scale(a,b,c,d=0,f=1){e({number:a}).lteq(f);e({number:a}).gteq(d);return this.scaleNorm((a-d)/(f-d),b,c)}scaleNorm(a,b,c){e({number:a}).betweenEq(0,1);return a*(c-b)+b}shouldThrowOnMaxRecursionsReached(a){return"boolean"===typeof a?(L(this,G,a,"f"),this):"undefined"!==typeof z(this,G,"f")?z(this,G,"f"):!0}normal({mean:a,stddev:b,max:c,min:d,skew:f=0}={},h=0){if("undefined"===typeof d&&"undefined"===typeof c)return this.gaussian({mean:a,
stddev:b,skew:f});if(500<h&&this.shouldThrowOnMaxRecursionsReached())throw new W(`Max recursive calls to rng normal function. This might be as a result of using predictable random numbers, or inappropriate arguments? Args: ${JSON.stringify({mean:a,stddev:b,max:c,min:d,skew:f})}`);let l=this.bates(7);l=0>f?1-Math.pow(l,Math.pow(2,f)):Math.pow(l,Math.pow(2,-f));if("undefined"===typeof a&&"undefined"===typeof b&&"undefined"!==typeof c&&"undefined"!==typeof d)return this.scaleNorm(l,d,c);"undefined"===
typeof a?(a=0,"undefined"!==typeof c&&"undefined"!==typeof d&&(a=(c+d)/2,"undefined"===typeof b&&(b=Math.abs(c-d)/10)),"undefined"===typeof b&&(b=.1)):"undefined"===typeof b&&(b="undefined"!==typeof c&&"undefined"!==typeof d?Math.abs(c-d)/10:.1);l=(10*l-5)*b+a;if(500>=h&&("undefined"!==typeof c&&l>c||"undefined"!==typeof d&&l<d))return this.normal({mean:a,stddev:b,max:c,min:d,skew:f},h+1);"undefined"!==typeof c&&(l=Math.min(l,c));"undefined"!==typeof d&&(l=Math.max(l,d));return l}gaussian({mean:a=
0,stddev:b=1,skew:c=0}={}){e({stddev:b}).positive();if(0===c)return this.boxMuller({mean:a,stddev:b});let d=this.boxMuller({mean:0,stddev:1});d=d/10+.5;d=0>c?1-Math.pow(d,Math.pow(2,c)):Math.pow(d,Math.pow(2,-c));return(10*d-5)*b+a}boxMuller(a=0,b=1){"object"===typeof a&&({mean:a=0,stddev:b=1}=a);e({stddev:b}).gteq(0);const c=1-this._random(),d=this._random();return Math.sqrt(-2*Math.log(c))*Math.cos(2*Math.PI*d)*b+a}irwinHall(a=6){"object"===typeof a&&({n:a=6}=a);e({n:a}).int().positive();let b=
0;for(let c=0;c<a;c++)b+=this._random();return b}bates(a=6){"object"===typeof a&&({n:a=6}=a);e({n:a}).int().positive();return this.irwinHall({n:a})/a}batesgaussian(a=6){"object"===typeof a&&({n:a=6}=a);e({n:a}).int().gt(1);return this.irwinHall({n:a})/Math.sqrt(a)-1/Math.sqrt(1/a)/2}bernoulli(a=.5){"object"===typeof a&&({p:a=.5}=a);e({p:a}).lteq(1).gteq(0);return this._random()<a?1:0}exponential(a=1){"object"===typeof a&&({rate:a=1}=a);e({rate:a}).gt(0);return-Math.log(1-this._random())/a}pareto({shape:a=
.5,scale:b=1,location:c=0}={}){e({shape:a}).gteq(0);e({scale:b}).positive();const d=this._random();return 0!==a?c+b/a*(Math.pow(d,-a)-1):c-b*Math.log(d)}poisson(a=1){"object"===typeof a&&({lambda:a=1}=a);e({lambda:a}).positive();a=Math.exp(-a);let b=0,c=1,d=0;const f=new K(10,2);do{b++;const h=this._random();f.push(h);c*=h;f.detectLoop(`Loop detected in randomly generated numbers over the last ${10} generations. This is incompatible with the poisson distribution. Try either using a spread of non-random numbers or fine tune the number to not fall foul of the looped way of generating. Last random number was ${h}`)}while(c>
a&&1E7>d++);if(1E7<=d+1)throw Error("LOOP_MAX reached in poisson - bailing out - possible parameter error, or using non-random source?");return b-1}hypergeometric({N:a=50,K:b=10,n:c=5,k:d}={}){function f(l){let q=0;for(let r=2;r<=l;r++)q+=Math.log(r);return q}function h(l,q){return f(l)-f(q)-f(l-q)}e({N:a}).int().positive();e({K:b}).int().positive().lteq(a);e({n:c}).int().positive().lteq(a);"undefined"===typeof d&&(d=this.randInt(0,Math.min(b,c)));e({k:d}).int().betweenEq(0,Math.min(b,c));a=h(b,d)+
h(a-b,c-d)-h(a,c);return Math.exp(a)}rademacher(){return.5>this._random()?-1:1}binomial({n:a=1,p:b=.5}={}){e({n:a}).int().positive();e({p:b}).betweenEq(0,1);let c=0;for(let d=0;d<a;d++)this._random()<b&&c++;return c}betaBinomial({alpha:a=1,beta:b=1,n:c=1}={}){e({alpha:a}).positive();e({beta:b}).positive();e({n:c}).int().positive();a=((d,f)=>{let h=this._random(),l=this._random();h=Math.pow(h,1/d);l=Math.pow(l,1/f);return h/(h+l)})(a,b);b=0;for(let d=0;d<c;d++)this._random()<a&&b++;return b}beta({alpha:a=
.5,beta:b=.5}={}){e({alpha:a}).positive();e({beta:b}).positive();const c=d=>{let f=0;for(let h=0;h<d;h++){const l=this._random();f+=-Math.log(l);if(1E7<=h+1)throw Error("LOOP_MAX reached in beta - bailing out - possible parameter error, or using non-random source?");}return f};a=c(a);b=c(b);return a/(a+b)}gamma({shape:a=1,rate:b,scale:c}={}){e({shape:a}).positive();if("undefined"!==typeof c&&"undefined"!==typeof b&&b!==1/c)throw Error("Cannot supply rate and scale");"undefined"!==typeof c&&(e({scale:c}).positive(),
b=1/c);"undefined"===typeof b&&(b=1);b&&e({rate:b}).positive();let d;let f=1;const h=a-1/3,l=1/Math.sqrt(9*h);let q=0;d=!0;const r=new K(10);for(;d&&1E7>q++;){var t=0;var u=new K(10);do{var A=this.normal();u.push(A);u.detectLoop("Loop detected in randomly generated numbers over the last 10 generations. This is incompatible with the gamma distribution. Try either using a spread of non-random numbers or fine tune the number to not fall foul ofthe looped way of generating.");f=1+l*A}while(0>=f&&1E7>
t++);if(1E7<=t+1)throw Error(`LOOP_MAX reached inside gamma inner loop - bailing out - possible parameter error, or using non-random source? had shape = ${a}, rate = ${b}, scale = ${c}`);f*=Math.pow(f,2);t=Math.pow(A,2);A=1-.331*t*t;t=.5*t+h*(1-f+Math.log(f));u=this._random();r.push(u);r.detectLoop(`Loop detected in randomly generated numbers over the last ${10} generations. This is incompatible with the gamma distribution. Try either using a spread of non-random numbers or fine tune the number to not fall foul of the looped way of generating. Last random number was ${u}`);
if(u<A||Math.log(u)<t)d=!1}if(1E7<=q+1)throw Error(`LOOP_MAX reached inside gamma - bailing out - possible parameter error, or using non-random source? had shape = ${a}, rate = ${b}, scale = ${c}`);return b*h*f}studentsT(a=1){"object"===typeof a&&({nu:a=1}=a);e({nu:a}).positive();const b=Math.sqrt(-2*Math.log(this._random()))*Math.cos(2*Math.PI*this._random()),c=this.gamma({shape:a/2,rate:2});return b/Math.sqrt(c/a)}wignerSemicircle(a=1){"object"===typeof a&&({R:a=1}=a);e({R:a}).gt(0);const b=2*this._random()*
Math.PI;return a*Math.cos(b)}kumaraswamy({alpha:a=.5,beta:b=.5}={}){e({alpha:a}).gt(0);e({beta:b}).gt(0);const c=this._random();return Math.pow(1-Math.pow(1-c,1/b),1/a)}hermite({lambda1:a=1,lambda2:b=2}={}){e({lambda1:a}).gt(0);e({lambda2:b}).gt(0);a=this.poisson({lambda:a});b=this.poisson({lambda:b});return a+b}chiSquared(a=1){"object"===typeof a&&({k:a=1}=a);e({k:a}).positive().int();let b=0;for(let c=0;c<a;c++){const d=Math.sqrt(-2*Math.log(this._random()))*Math.cos(2*Math.PI*this._random());b+=
d*d}return b}rayleigh(a=1){"object"===typeof a&&({scale:a=1}=a);e({scale:a}).gt(0);return a*Math.sqrt(-2*Math.log(this._random()))}logNormal({mean:a=0,stddev:b=1}={}){e({stddev:b}).gt(0);a+=b*Math.sqrt(-2*Math.log(this._random()))*Math.cos(2*Math.PI*this._random());return Math.exp(a)}cauchy({median:a=0,scale:b=1}={}){e({scale:b}).gt(0);const c=this._random();return a+b*Math.tan(Math.PI*(c-.5))}laplace({mean:a=0,scale:b=1}={}){e({scale:b}).gt(0);const c=this._random()-.5;return a-b*Math.sign(c)*Math.log(1-
2*Math.abs(c))}logistic({mean:a=0,scale:b=1}={}){e({scale:b}).gt(0);const c=this._random();return a+b*Math.log(c/(1-c))}support(a){return{random:"[min, max)",integer:"[min, max]",normal:"(-INF, INF)",boxMuller:"(-INF, INF)",gaussian:"(-INF, INF)",irwinHall:"[0, n]",bates:"[0, 1]",batesgaussian:"(-INF, INF)",bernoulli:"{0, 1}",exponential:"[0, INF)",pareto:"[scale, INF)",poisson:"{1, 2, 3 ...}",hypergeometric:"{max(0, n+K-N), ..., min(n, K)}",rademacher:"{-1, 1}",binomial:"{0, 1, 2, ..., n}",betaBinomial:"{0, 1, 2, ..., n}",
beta:"(0, 1)",gamma:"(0, INF)",studentsT:"(-INF, INF)",wignerSemicircle:"[-R; +R]",kumaraswamy:"(0, 1)",hermite:"{0, 1, 2, 3, ...}",chiSquared:"[0, INF)",rayleigh:"[0, INF)",logNormal:"(0, INF)",cauchy:"(-INF, +INF)",laplace:"(-INF, +INF)",logistic:"(-INF, +INF)"}[a]}chancyInt(a){var b;if("number"===typeof a)return Math.round(a);if(Array.isArray(a)){for(var c of a)if(!n(c))throw Error("Cannot pass non-numbers to chancyInt");a=this.choice(a);"number"!==typeof a&&(a=parseFloat(a));return Math.round(a)}"object"===
typeof a&&(a=Object.assign({},a),c=null!==(b=a.type)&&void 0!==b?b:"random","random"===c?a.type="integer":"normal"===c&&(a.type="normal_integer"));return Math.round(this.chancy(a))}chancy(a,b=0){var c,d;if(500<=b){if(this.shouldThrowOnMaxRecursionsReached())throw new W("Max recursions reached in chancy. Usually a case of badly chosen min/max values.");return 0}if(Array.isArray(a))return this.choice(a);if("string"===typeof a)return this.dice(a);if("object"===typeof a){a=Object.assign({},a);a.type=
null!==(c=a.type)&&void 0!==c?c:"random";"random"!==a.type&&"int"!==a.type&&"integer"!==a.type||"undefined"===typeof a.min||"undefined"!==typeof a.max||(a.max=Number.MAX_SAFE_INTEGER);switch(a.type){case "random":return this.random(a.min,a.max,a.skew);case "int":case "integer":return this.randInt(a.min,a.max,a.skew);case "normal_integer":case "normal_int":return Math.round(this.normal(a));case "dice":return this.chancyMinMax(this.dice(null!==(d=a.dice)&&void 0!==d?d:a),a,b);case "rademacher":return this.chancyMinMax(this.rademacher(),
a,b);case "normal":case "gaussian":case "boxMuller":case "irwinHall":case "bates":case "batesgaussian":case "bernoulli":case "exponential":case "pareto":case "poisson":case "hypergeometric":case "binomial":case "betaBinomial":case "beta":case "gamma":case "studentsT":case "wignerSemicircle":case "kumaraswamy":case "hermite":case "chiSquared":case "rayleigh":case "logNormal":case "cauchy":case "laplace":case "logistic":return this.chancyMinMax(this[a.type](a),a,b)}throw Error(`Invalid input type given to chancy: "${a.type}".`);
}if("number"===typeof a)return a;throw Error("Invalid input given to chancy");}chancyMinMax(a,b,c=0){const {min:d,max:f}=b;return 500<=c+1&&!this.shouldThrowOnMaxRecursionsReached()?("undefined"!==typeof d&&(a=Math.max(d,a)),"undefined"!==typeof f&&(a=Math.min(f,a)),a):"undefined"!==typeof d&&a<d||"undefined"!==typeof f&&a>f?this.chancy(b,c+1):a}chancyMin(a){const {constructor:b}=Object.getPrototypeOf(this);return b.chancyMin(a)}chancyMax(a){const {constructor:b}=Object.getPrototypeOf(this);return b.chancyMax(a)}static chancyMin(a){var b,
c,d,f,h,l,q;if(Array.isArray(a)){for(const r of a)if(!n(r))throw Error("Cannot pass non-numbers to chancyMin array input");return Math.min(...a)}if("string"===typeof a)return this.diceMin(a);if("number"===typeof a)return a;if("object"===typeof a){a.type=null!==(b=a.type)&&void 0!==b?b:"random";"random"!==a.type&&"integer"!==a.type||"undefined"===typeof a.min||"undefined"!==typeof a.max||(a.max=Number.MAX_SAFE_INTEGER);switch(a.type){case "dice":return this.diceMin(a.dice);case "normal":return null!==
(c=a.min)&&void 0!==c?c:Number.NEGATIVE_INFINITY;case "normal_integer":return null!==(d=a.min)&&void 0!==d?d:Number.NEGATIVE_INFINITY;case "integer":return null!==(f=a.min)&&void 0!==f?f:0;case "random":return null!==(h=a.min)&&void 0!==h?h:0;case "boxMuller":return Number.NEGATIVE_INFINITY;case "gaussian":return Number.NEGATIVE_INFINITY;case "irwinHall":return 0;case "bates":return 0;case "batesgaussian":return Number.NEGATIVE_INFINITY;case "bernoulli":return 0;case "exponential":return 0;case "pareto":return null!==
(l=a.scale)&&void 0!==l?l:1;case "poisson":return 1;case "hypergeometric":const {N:r=50,K:t=10,n:u=5}=a;return Math.max(0,u+t-r);case "rademacher":return-1;case "binomial":return 0;case "betaBinomial":return 0;case "beta":return Number.EPSILON;case "gamma":return Number.EPSILON;case "studentsT":return Number.NEGATIVE_INFINITY;case "wignerSemicircle":return-1*(null!==(q=a.R)&&void 0!==q?q:10);case "kumaraswamy":return Number.EPSILON;case "hermite":return 0;case "chiSquared":return 0;case "rayleigh":return 0;
case "logNormal":return Number.EPSILON;case "cauchy":return Number.NEGATIVE_INFINITY;case "laplace":return Number.NEGATIVE_INFINITY;case "logistic":return Number.NEGATIVE_INFINITY}throw Error(`Invalid input type ${a.type}.`);}throw Error("Invalid input supplied to chancyMin");}static chancyMax(a){var b,c,d,f,h,l,q,r,t;if(Array.isArray(a)){for(const u of a)if(!n(u))throw Error("Cannot pass non-numbers to chancyMax array input");return Math.max(...a)}if("string"===typeof a)return this.diceMax(a);if("number"===
typeof a)return a;if("object"===typeof a){a.type=null!==(b=a.type)&&void 0!==b?b:"random";"random"!==a.type&&"integer"!==a.type||"undefined"===typeof a.min||"undefined"!==typeof a.max||(a.max=Number.MAX_SAFE_INTEGER);switch(a.type){case "dice":return this.diceMax(a.dice);case "normal":return null!==(c=a.max)&&void 0!==c?c:Number.POSITIVE_INFINITY;case "normal_integer":return null!==(d=a.max)&&void 0!==d?d:Number.POSITIVE_INFINITY;case "integer":return null!==(f=a.max)&&void 0!==f?f:1;case "random":return null!==
(h=a.max)&&void 0!==h?h:1;case "boxMuller":return Number.POSITIVE_INFINITY;case "gaussian":return Number.POSITIVE_INFINITY;case "irwinHall":return null!==(l=a.n)&&void 0!==l?l:6;case "bates":return 1;case "batesgaussian":return Number.POSITIVE_INFINITY;case "bernoulli":return 1;case "exponential":return Number.POSITIVE_INFINITY;case "pareto":return Number.POSITIVE_INFINITY;case "poisson":return Number.MAX_SAFE_INTEGER;case "hypergeometric":const {K:u=10,n:A=5}=a;return Math.min(A,u);case "rademacher":return 1;
case "binomial":return null!==(q=a.n)&&void 0!==q?q:1;case "betaBinomial":return null!==(r=a.n)&&void 0!==r?r:1;case "beta":return 1;case "gamma":return Number.POSITIVE_INFINITY;case "studentsT":return Number.POSITIVE_INFINITY;case "wignerSemicircle":return null!==(t=a.R)&&void 0!==t?t:10;case "kumaraswamy":return 1;case "hermite":return Number.MAX_SAFE_INTEGER;case "chiSquared":return Number.POSITIVE_INFINITY;case "rayleigh":return Number.POSITIVE_INFINITY;case "logNormal":return Number.POSITIVE_INFINITY;
...
typeof c&&(a=a<=c?a:c);"undefined"!==typeof b&&(a=a>=b?a:b);return a}bin(a,b,c,d){e({val:a}).gt(c).lt(d);d-=c;return Math.round((a-c)/d*(b-1))/(b-1)*d+c}shuffle(a){a=[...a];for(let b=a.length-1;0<b;b--){const c=Math.floor(this.random()*(b+1));[a[b],a[c]]=[a[c],a[b]]}return a}}var F=new WeakMap;var D=new WeakMap;var G=new WeakMap;var da=new WeakMap;O.version="0.2.3";class E extends O{static predictable(a){return new this(null!==a&&void 0!==a?a:5789938451)}serialize(){return{seed:this.getSeed(),version:"0.2.3"}}sameAs(a){return a instanceof
E?this.getRandomSource()===a.getRandomSource()&&this.getSeed()===a.getSeed():!1}from(a){this.setSeed(a.getSeed());return this}_next(){this.setSeed(this.getSeed()+2654435769|0);let a=this.getSeed();a=Math.imul(a^a>>>16,569420461);a=Math.imul(a^a>>>15,1935289751);return((a^a>>>15)>>>0)/4294967296}rotCompatible(){const a=this;return{getUniform(){return a.random()},getUniformInt(b,c){return a.randInt(b,c)},getNormal(b=0,c=1){return a.boxMuller({mean:b,stddev:c})},getPercentage(){return 1+Math.floor(100*
this.getUniform())},getItem(b){return a.choice(b)},shuffle(b){return a.shuffle(b)},getWeightedValue(b){return a.weightedChoice(b)},getSeed(){return a.getSeed()},setSeed(b){a.setSeed(b);return this},getState(){return a.serialize()},setState(b){a.setSeed(b.seed);return this},clone(){return(new E).rotCompatible().setState(this.getState())}}}}return E}();
